...
class LearnedCoreset(AttentionAlgorithm):
    ...
    def __init__(
        ...
        if init not in ("kmeans", "mqbeta", "first", "random", "random_gauss"):
            raise ValueError(f"invalid init {init!r}")
        self.init = init
        self.lr = float(lr)
        ...
        # budget -> {"train_loss": [...], "val_loss": [...]}
        self._training_history: Dict[int, Dict] = {}
    ...
    def _get_learned(
        ...
        k_prime, v_prime, w_prime, hist = result
        self._learned_cache[budget] = (k_prime, v_prime, w_prime)
        self._training_history[budget] = hist
