...
Same pipeline as FCFW-l2 / WildCat2 (full-sequence key centering, sink + local
window via WtdAttn with weight 1, shared temperature), but the coreset is
selected to approximate the *tensor* target sum_i phi(k_i) (x) [1, v_i] -- i.e.
value-aware selection -- and the synthetic coreset values are produced by the
algorithm's own fully-corrective tensor solve. Deterministic and nested.
"""
...
class TensorFCFWL2(AttentionAlgorithm):
    """Tensor FCFW (l2) coreset on candidate keys; sink + window via WtdAttn."""
    ...
    def name(self) -> str:
        if self.bin_r is not None:
            return f"TFCFW-l2-bin{self.bin_r}"
        if self.num_bins > 1:
            return f"TFCFW-l2-C{self.num_bins}"
        return "TFCFW-l2"
    ...
    def _compress_single(
        ...
        (ck, cv, w, loc), new_state = compress_kv_tensor_fcfw(
            keys, values, r, scale=scale, q_scale=q_scale,
            phi=self.phi, oracle=self.oracle, state=self._cache_state,
        )
    ...
    def _compress_binned(
        ...
        for _ in range(n_bins):
            ...
            (ck, cv, w, loc), _ = compress_kv_tensor_fcfw(
                keys[..., sl, :], values[..., sl, :], per_r,
                scale=scale, q_scale=q_scale, phi=self.phi,
                oracle=self.oracle, state=None,
            )
            ck_parts.append(ck)
            cv_parts.append(cv)
            w_parts.append(w)
